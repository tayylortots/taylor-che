...
export function Footer({ onScrollToTop, isRevealed }: { onScrollToTop: () => void; isRevealed: boolean }) {
  ...
  return (
    <motion.div
      className="w-full h-full min-h-[90vh] relative overflow-hidden rounded-t-[20px] flex flex-col items-center justify-center" style={{ scrollSnapAlign: 'start', scrollSnapStop: 'always' }}
      animate={{ backgroundColor: isRevealed ? '#5b8da3' : '#757575' }}
    ...
    >
      ...
      <motion.div
        ...
        initial={false}
        animate={isRevealed ? { x: 0, opacity: 1 } : { x: '-120%', opacity: 0 }}
        transition={{ duration: 0.9, ease: 'easeOut', delay: isRevealed ? 0.25 : 0 }}
      >
      ...
      <motion.div
        ...
        initial={false}
        animate={isRevealed ? { x: 0, opacity: 1 } : { x: '220%', opacity: 0 }}
        transition={{ duration: 0.9, ease: 'easeOut', delay: isRevealed ? 0.35 : 0 }}
      >
      ...
      <motion.div
        ...
        animate={isRevealed ? { opacity: isRevealed ? 0 : 1, y: 0 } : { opacity: 0, y: 20 }}
      ...
      <motion.div
        className="absolute left-1/2 top-[22%] sm:top-[15%] -translate-x-1/2 -translate-y-1/2 z-0 text-center pointer-events-none select-none w-full px-4"
        initial={{ opacity: 0, y: 20 }}
        animate={isRevealed ? { opacity: isRevealed ? 1 : 0, y: 0 } : { opacity: 0, y: 20 }}
        transition={{ duration: 0.8, ease: 'easeOut', delay: 0 }}
      >
      ...
      <motion.div
        ...
        animate={isRevealed ? { opacity: 1, x: 0, color: isRevealed ? '#fefed6' : '#cecece' } : { opacity: 0, x: -20 }}
      ...
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="sm:w-[22px] sm:h-[22px] md:w-[24px] md:h-[24px]">
          <motion.path
            d="M12 19V5"
            animate={{ stroke: isRevealed ? '#fefed5' : '#cecece' }}
        ...
        <motion.span
          ...
          animate={{ color: isRevealed ? '#fefed5' : '#cecece' }}
          transition={{ duration: 1.2 }}
        >
          back to top
        </motion.span>
